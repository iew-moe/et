...
function renderTasks(c) {
  ...
  if (c.mode === "dimension") {
    ...
    tasks.push(`1) ${tr("taskFormula").replace("{RV}", rv)}`);
    tasks.push(`2) ${tr("taskDimA")}`);
    tasks.push(`3) ${tr("taskI0")}`);
  } else {
    tasks.push(`1) ${tr("taskR20")}`);
    tasks.push(`2) ${tr("taskS0")}`);
    if (c.source_type === "V") {
      tasks.push(`3) ${tr("taskI0")}`);
    } else {
      tasks.push(`3) ${tr("taskVRL")}`);
    }
  }
  tasks.push(`4) ${tr("taskPL")}`);
  if (hasLoad) {
    tasks.push(`5) ${tr("taskPV")}`);
    tasks.push(`6) ${tr("taskP0")}`);
    tasks.push(`7) ${tr("taskRT")}`);
    tasks.push(`8) ${tr("taskType")}`);
  } else {
    tasks.push(`5) ${tr("taskP0")}`);
    tasks.push(`6) ${tr("taskRT")}`);
    tasks.push(`7) ${tr("taskType")}`);
  }
  document.getElementById("tasks").innerHTML =
    `<div class="section-title">${tr("tasksTitle")}</div>` + tasks.map((t) => `${t}<br>`).join("");
}
...
function renderInputs(c) {
  ...
  const rows = [];
  if (c.mode === "basis") {
    rows.push(makeInputRow(`1) ${tr("inR20")}:`, "in_R20"));
    rows.push(makeInputRow(`2) ${tr("inS0")}:`, "in_S20"));
    if (c.source_type === "V") {
      rows.push(makeInputRow(`3) ${tr("inI0")}:`, "in_I0"));
    } else {
      rows.push(makeInputRow(`3) ${tr("inVRL")}:`, "in_VRL"));
    }
  } else {
    rows.push(`<div class="row"><div class="label"><b>1) ${tr("inputsFormula")}</b></div></div>`);
    rows.push(makeInputRow(`2) ${tr("inA")}:`, "in_A"));
    rows.push(makeInputRow(`3) ${tr("inI0")}:`, "in_I0"));
  }
  rows.push(makeInputRow(`4a) ${tr("inPL")}:`, "in_PL"));
  if (hasLoad) rows.push(makeInputRow(`4b) ${tr("inPV")}:`, "in_PV"));
  rows.push(makeInputRow(`${hasLoad ? "4c" : "4b"}) ${tr("inP0")}:`, "in_P0"));
  rows.push(makeInputRow(`5) ${tr("inRT")}:`, "in_RT"));
  rows.push(
    `<div class="row"><label class="label" id="lbl-nature">${tr("nature")}</label>` +
